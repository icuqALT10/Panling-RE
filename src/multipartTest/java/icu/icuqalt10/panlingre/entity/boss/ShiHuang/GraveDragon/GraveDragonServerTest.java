...
public final class GraveDragonServerTest {
    ...
    /** 当前姿态下所有部件 OBB 与方块的相交深度（诊断用：为 0 说明"卡住"不是被方块挡的）。 */
    private static double worstPartPenetration(net.minecraft.world.level.Level level, GraveDragonEntity dragon) {
        double worst = 0;
        for (var part : dragon.getWorldParts()) {
            var box = part.getOrientedBox();
            if (box == null) continue;
            for (var shape : level.getBlockCollisions(dragon, box.enclosingAabb())) {
                for (var block : shape.toAabbs()) {
                    if (box.intersects(block)) {
                        worst = Math.max(worst, Math.min(box.enclosingAabb().getYsize(), block.getYsize()));
                    }
                }
            }
        }
