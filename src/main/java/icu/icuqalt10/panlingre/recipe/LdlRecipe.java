...
public record LdlRecipe(List<SlotIngredient> slotIngredients, ItemStack result) implements Recipe<RecipeInput> {
    ...
    public boolean matches(RecipeInput input, Level level) {
        ...
        for (SlotIngredient si : slotIngredients) {
            ...
            if (targetSlot < 0 || targetSlot >= 5) continue;
    ...
    public RecipeSerializer<?> getSerializer() {
        return ModRecipes.LDL_SERIALIZER.get();
    }

    @Override
    public RecipeType<?> getType() {
        return ModRecipes.LDL_TYPE.get();
    }

    public static class Serializer implements RecipeSerializer<LdlRecipe> {
        public static final MapCodec<LdlRecipe> CODEC = RecordCodecBuilder.mapCodec(inst -> inst.group(
                // 注意这里字段名改为 "ingredients" 以匹配你的 JSON
                SlotIngredient.CODEC.codec().listOf().fieldOf("ingredients").forGetter(LdlRecipe::slotIngredients),
                ItemStack.CODEC.fieldOf("result").forGetter(LdlRecipe::result)
        ).apply(inst, LdlRecipe::new));

        public static final StreamCodec<RegistryFriendlyByteBuf, LdlRecipe> STREAM_CODEC = StreamCodec.composite(
                SlotIngredient.STREAM_CODEC.apply(ByteBufCodecs.collection(java.util.ArrayList::new)),
                LdlRecipe::slotIngredients,
        ...
        @Override
        public MapCodec<LdlRecipe> codec() {
            return CODEC;
        }

        @Override
        public StreamCodec<RegistryFriendlyByteBuf, LdlRecipe> streamCodec() {
            return STREAM_CODEC;
        }
    }
}