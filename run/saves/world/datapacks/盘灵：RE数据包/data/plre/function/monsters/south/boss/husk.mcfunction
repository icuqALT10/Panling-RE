execute store result score @s monster_count run execute if entity @e[tag=monster,distance=..10]

execute if score @s monster_count matches 9.. run return 0

summon husk ~ ~ ~ {CanPickUpLoot:false,ArmorDropChances:[0f,0f,0f,0f],HandDropChances:[0f,0f],Health:10000000000.0f,\
Tags:["monster","south","boss","husk"],\
CustomNameVisible:1b,CustomName:'{"translate":"plre.monster.south.bosshusk"}',\
DeathLootTable:"plre:monsters/south/boss/husk",\
attributes:\
[\
{id:"generic.max_health",base:500},\
{id:"generic.movement_speed",base:0.4},\
{id:"generic.minecraft:attack_damage",base:4},\
{id:"generic.armor",base:0d},\
{id:"generic.follow_range",base:40d},\
],\
ArmorItems:\
[\
{},\
{},\
{},\
{id:"iron_helmet",components:{unbreakable:{}}}\
],\
HandItems:\
[\
{id:"diamond_sword",count:1b,components:{unbreakable:{},enchantments:{fire_aspect:2,knockback:1}}},\
{}\
],\
Team:"monster",PersistenceRequired:0b}